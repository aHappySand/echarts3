...
function getCoordSysDimDefs(seriesModel, coordSysInfo) {
  ...
  if (coordSysInfo && coordSysInfo.coordSysDims) {
    coordSysDimDefs = zrUtil.map(coordSysInfo.coordSysDims, (dim) => {
      var dimInfo = {
        name: dim
      };
...
function injectOrdinalMeta(dimInfoList, createInvertedIndices, coordSysInfo) {
  ...
  var hasNameEncode;
  coordSysInfo && zrUtil.each(dimInfoList, (dimInfo, dimIndex) => {
...
function createSeriesData(sourceRaw, seriesModel, opt) {
  ...
  var encodeDefaulter = zrUtil.isFunction(useEncodeDefaulter)
    ? useEncodeDefaulter
    : useEncodeDefaulter
  ...
  var stackCalculationInfo = enableDataStack(seriesModel, { schema, store });
