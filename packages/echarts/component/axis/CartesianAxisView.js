...
const CartesianAxisView = /** @class */ (function (_super) {
  ...
  CartesianAxisView.prototype.render = function (axisModel, ecModel, api, payload) {
    ...
    this._axisGroup = new graphic.Group3D();
    ...
    zrUtil.each(selfBuilderAttrs, function (name) {
      if (axisModel.get([name, 'show'])) {
        const p = axisElementBuilders[name](this, this._axisGroup, axisModel, gridModel);
        if (p) {
...
const axisElementBuilders = {
  splitLine(axisView, axisGroup, axisModel, gridModel) {
    ...
    let color = lineStyle.color || lineStyle.stroke;
    if (color instanceof Array) {
      color = color[0];
    }
    if (color) {
      axisView.axisLineMaterial.color = new THREE.Color(color);
    }
    const positions = [];
...
const CartesianXAxisView = /** @class */ (function (_super) {
...
const CartesianYAxisView = /** @class */ (function (_super) {
