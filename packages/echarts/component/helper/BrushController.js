...
import { curry, each, map, bind, merge, clone, defaults, assert } from '../../../zrender/core/util';
 ...
 *     }
 */
const BrushController = /** @class */ (function (_super) {
  ...
  BrushController.prototype._doEnableBrush = function (brushOption) {
    ...
    each(this._handlers, (handler, eventName) => {
  ...
  BrushController.prototype.updateCovers = function (coverConfigList) {
    ...
    coverConfigList = map(coverConfigList, (coverConfig) => merge(clone(DEFAULT_BRUSH_OPT), coverConfig, true));
...
function trigger(controller, opt) {
  var areas = map(controller._covers, (cover) => {
    var brushOption = cover.__brushOption;
...
function createBaseRectCover(rectRangeConverter, controller, brushOption, edgeNameSequences) {
  ...
  each(edgeNameSequences, (nameSequence) => {
    cover.add(new graphic.Rect({
...
function updateCommon(controller, cover) {
  ...
  each([['w'], ['e'], ['n'], ['s'], ['s', 'e'], ['s', 'w'], ['n', 'e'], ['n', 'w']], (nameSequence) => {
...
const coverRenderers = {
  ...
  rect: {
    createCover(controller, brushOption) {
    ...
    getCreatingRange(localTrack) {
      var ends = getTrackEnds(localTrack);
      return formatRectRange(ends[1][0], ends[1][1], ends[0][0], ends[0][1]);
    },
    updateCoverShape(controller, cover, localRange, brushOption) {
