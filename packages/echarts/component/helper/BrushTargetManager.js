...
* under the License.
*/
import { each, indexOf, curry, assert, map, createHashMap } from '../../../zrender/core/util';
...
const BrushTargetManager = /** @class */ (function () {
  /**
  ...
  BrushTargetManager.prototype.setOutputRanges = function (areas, ecModel) {
    this.matchOutputRanges(areas, ecModel, (area, coordRange, coordSys) => {
  ...
  BrushTargetManager.prototype.matchOutputRanges = function (areas, ecModel, cb) {
    each(areas, function (area) {
      ...
      if (targetInfo && targetInfo !== true) {
        each(targetInfo.coordSyses, (coordSys) => {
  ...
  BrushTargetManager.prototype.makePanelOpts = function (api, getDefaultBrushType) {
    return map(this._targetInfoList, (targetInfo) => {
...
function parseFinder(ecModel, finder) {
  return modelUtilParseFinder(ecModel, finder, { includeMainTypes: INCLUDE_FINDER_MAIN_TYPES });
}
const targetInfoBuilders = {
  grid(foundCpts, targetInfoList) {
    ...
    each(xAxisModels, (axisModel) => {
      var gridModel = axisModel.axis.grid.model;
      gridModelMap.set(gridModel.id, gridModel);
      xAxesHas[gridModel.id] = true;
    });
    each(yAxisModels, (axisModel) => {
      var gridModel = axisModel.axis.grid.model;
      gridModelMap.set(gridModel.id, gridModel);
      yAxesHas[gridModel.id] = true;
    });
    each(gridModels, (gridModel) => {
      gridModelMap.set(gridModel.id, gridModel);
      xAxesHas[gridModel.id] = true;
      yAxesHas[gridModel.id] = true;
    });
    gridModelMap.each((gridModel) => {
      ...
      var cartesians = [];
      each(grid.getCartesians(), (cartesian, index) => {
  ...
  geo(foundCpts, targetInfoList) {
    each(foundCpts.geoModels, (geoModel) => {
...
const coordConvert = {
  ...
  rect(to, coordSys, rangeOrCoordRange, clamp) {
    var xminymin = to
  ...
  polygon(to, coordSys, rangeOrCoordRange, clamp) {
    var xyMinMax = [[Infinity, -Infinity], [Infinity, -Infinity]];
    var values = map(rangeOrCoordRange, (item) => {
...
function axisConvert(axisNameIndex, to, coordSys, rangeOrCoordRange) {
  ...
  var values = formatMinMax(map([0, 1], (i) => (to
...
const diffProcessor = {
  ...
  rect(values, refer, scales) {
    return [
      [values[0][0] - scales[0] * refer[0][0], values[0][1] - scales[0] * refer[0][1]],
      [values[1][0] - scales[1] * refer[1][0], values[1][1] - scales[1] * refer[1][1]]
    ];
  },
  polygon(values, refer, scales) {
    return map(values, (item, idx) => [item[0] - scales[0] * refer[idx][0], item[1] - scales[1] * refer[idx][1]]);
  }
};
