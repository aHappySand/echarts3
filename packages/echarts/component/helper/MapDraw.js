...
import Displayable from '../../../zrender/graphic/Displayable';
 ...
 */
const OPTION_STYLE_ENABLED_TAGS = [
  'rect', 'circle', 'line', 'ellipse', 'polygon', 'polyline', 'path'
];
const OPTION_STYLE_ENABLED_TAG_MAP = zrUtil.createHashMap(OPTION_STYLE_ENABLED_TAGS);
const STATE_TRIGGER_TAG_MAP = zrUtil.createHashMap(OPTION_STYLE_ENABLED_TAGS.concat(['g']));
const LABEL_HOST_MAP = zrUtil.createHashMap(OPTION_STYLE_ENABLED_TAGS.concat(['g']));
...
const MapDraw = /** @class */ (function () {
  ...
  MapDraw.prototype.draw = function (mapOrGeoModel, ecModel, api, fromView, payload) {
    ...
    isGeo && ecModel.eachComponent({ mainType: 'series', subType: 'map' }, (mapSeries) => {
  ...
  MapDraw.prototype._buildGeoJSON = function (viewBuildCtx) {
    ...
    zrUtil.each(viewBuildCtx.geo.regions, (region) => {
      ...
      var _a = regionsInfoByName.get(regionName) || {}; var dataIdx = _a.dataIdx; var
      ...
      if (!regionGroup) {
        ...
        regionModel = viewBuildCtx.isGeo
          ...
          : (data ? data.getItemModel(dataIdx) : null);
        regionsInfoByName.set(regionName, { dataIdx, regionModel });
      }
      var polygonSubpaths = [];
      var polylineSubpaths = [];
      zrUtil.each(region.geometries, (geometry) => {
    ...
    regionsGroupByName.each((regionGroup, regionName) => {
      var _a = regionsInfoByName.get(regionName); var dataIdx = _a.dataIdx; var
        regionModel = _a.regionModel;
  ...
  MapDraw.prototype._buildSVG = function (viewBuildCtx) {
    ...
    var focusSelf = false;
    zrUtil.each(this._svgGraphicRecord.named, (namedItem) => {
  ...
  MapDraw.prototype._updateController = function (mapOrGeoModel, ecModel, api) {
    ...
    controller.setPointerChecker((e, x, y) => geo.containPoint([x, y]) &&
  ...
  MapDraw.prototype._updateMapSelectHandler = function (mapOrGeoModel, regionsGroup, api, fromView) {
    ...
    if (mapOrGeoModel.get('selectedMode')) {
      regionsGroup.on('mousedown', () => {
        mapDraw._mouseDownFlag = true;
      });
      regionsGroup.on('click', (e) => {
