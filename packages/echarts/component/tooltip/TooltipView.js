...
* under the License.
*/
import { bind, each, clone, trim, isString, isFunction, isArray, isObject, extend } from '../../../zrender/core/util';
...
import { convertToColorString, formatTpl } from '../../util/format';
import { parsePercent } from '../../util/number';
import { ZRRect } from '../../util/graphic';
...
const proxyRect = new ZRRect({
  shape: { x: -1, y: -1, width: 2, height: 2 }
});
const TooltipView = /** @class */ (function (_super) {
  ...
  TooltipView.prototype._showAxisTooltip = function (dataByCoordSys, e) {
    ...
    each(dataByCoordSys, (itemCoordSys) => {
      each(itemCoordSys.dataByAxis, (axisItem) => {
        ...
        each(axisItem.seriesDataIndices, (idxItem) => {
  ...
  TooltipView.prototype._showSeriesItemTooltip = function (e, dispatcher, dispatchAction) {
    ...
    var markupText = frag ? buildTooltipMarkup(valueFormatter ? extend({ valueFormatter }, frag) : frag, markupStyleCreator, renderMode, orderMode, ecModel.get('useUTC'), tooltipModel.get('textStyle'))
...
function findComponentReference(payload, ecModel, api) {
  ...
  var queryResult = queryReferringComponents(ecModel, componentMainType, queryOptionMap.get(componentMainType), { useDefault: false, enableAll: false, enableNone: false });
