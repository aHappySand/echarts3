...
* under the License.
*/
import { isString, each, hasOwn, isArray, map, assert, extend } from '../../../zrender/core/util';
...
function buildSection(ctx, fragment, topMarginForOuterGap, toolTipTextStyle) {
  ...
  if (fragment.sortBlocks && orderMode) {
    ...
    if (hasOwn(orderMap, orderMode)) {
      ...
      subBlocks.sort((a, b) => comparator_1.evaluate(a.sortParam, b.sortParam));
    }
  ...
  each(subBlocks, (subBlock, idx) => {
    var valueFormatter = fragment.valueFormatter;
    var subMarkupText = getBuilder(subBlock)(
      // Inherit valueFormatter
      valueFormatter ? extend(extend({}, ctx), { valueFormatter }) : ctx, subBlock, idx > 0 ? gaps.html : 0, toolTipTextStyle
    );
    subMarkupText != null && subMarkupTextList.push(subMarkupText);
  });
  var subMarkupText = ctx.renderMode === 'richText'
...
function buildNameValue(ctx, fragment, topMarginForOuterGap, toolTipTextStyle) {
  ...
  var _a = getTooltipTextStyle(toolTipTextStyle, renderMode); var nameStyle = _a.nameStyle; var
