...
function makeCategoryLabelsActually(axis, labelModel) {
  ...
  return listCacheSet(labelsCache, optionLabelInterval, {
    labels, labelCategoryInterval: numericLabelInterval
  });
}
...
function makeCategoryTicks(axis, tickModel) {
  ...
  // labels. `splitLine` and `axisTick` should be consistent in this case.
    ...
    ticks = zrUtil.map(labelsResult.labels, (labelItem) => labelItem.tickValue);
  } else {
...
function makeRealNumberLabels(axis) {
  ...
  return {
    labels: zrUtil.map(ticks, (tick, idx) => ({
      level: tick.level,
...
export function calculateCategoryInterval(axis) {
  ...
  for (; tickValue <= ordinalExtent[1]; tickValue += step) {
    ...
    var rect = textContain.getBoundingRect(labelFormatter({ value: tickValue }), params.font, 'center', 'top');
    // Magic number
...
function makeLabelsByCustomizedCategoryInterval(axis, categoryInterval, onlyTick) {
  ...
  var labelFormatter = makeLabelFormatter(axis);
  var result = [];
  zrUtil.each(ordinalScale.getTicks(), (tick) => {
