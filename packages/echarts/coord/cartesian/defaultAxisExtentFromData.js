...
* under the License.
*/
import { createHashMap, each, hasOwn, keys, map } from 'zrender/src/core/util';
...
function calculateFilteredExtent(axisRecordMap, seriesRecords) {
  each(seriesRecords, (seriesRecord) => {
    ...
    function addCondition(axis, axisRecord) {
      ...
      if (axis.type === 'category' &&
        ...
        each(getDataDimensionsOnAxis(data, axis.dim), (dataDim) => {
    ...
    var tarDimExtents = map(tarDims, () => initExtent());
    ...
    each(tarDimExtents, (tarDimExtent, i) => {
      var dim = tarDims[i];
