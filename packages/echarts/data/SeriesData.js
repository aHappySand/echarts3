...
const SeriesData = /** @class */ (function () {
  ...
  SeriesData.prototype.initData = function (data, nameList, dimValueGetter) {
    ...
    if (!store) {
      ...
      var provider = (isSourceInstance(data) || zrUtil.isArrayLike(data))
        ? new DefaultDataProvider(data, dimensions.length)
        : data;
      store = new DataStore();
      var dimensionInfos = map(dimensions, (dimName) => ({
  ...
  SeriesData.prototype.appendValues = function (values, names) {
    var _a = this._store.appendValues(values, names.length); var start = _a.start; var
      end = _a.end;
  ...
  SeriesData.prototype.getValues = function (dimensions, idx) {
    ...
    return zrUtil.isArray(dimensions)
      ? store.getValues(map(dimensions, (dim) => _this._getStoreDimIndex(dim)), idx)
  ...
  SeriesData.prototype.diff = function (otherList) {
    var thisList = this;
    return new DataDiffer(otherList ? otherList.getStore().getIndices() : [], this.getStore().getIndices(), ((idx) => getId(otherList, idx)), ((idx) => getId(thisList, idx)));
  };
  /**
   * Get visual property.
   */
  ...
  SeriesData.prototype.eachItemGraphicEl = function (cb, context) {
    zrUtil.each(this._graphicEls, (el, idx) => {
      if (el) {
  ...
  SeriesData.prototype.cloneShallow = function (list) {
    if (!list) {
      list = new SeriesData(this._schema
        ? this._schema
  ...
  SeriesData.internalField = (function () {
    prepareInvertedIndex = function (data) {
      ...
      zrUtil.each(invertedIndicesMap, (invertedIndices, dim) => {
    ...
    cloneListForMapAndSample = function (original) {
      var list = new SeriesData(original._schema
        ? original._schema
        : map(original.dimensions, original._getDimInfo, original), original.hostModel);
      // FIXME If needs stackedOn, value may already been stacked
      transferProperties(list, original);
      return list;
    };
    transferProperties = function (target, source) {
      zrUtil.each(TRANSFERABLE_PROPERTIES.concat(source.__wrappedMethods || []), (propName) => {
        if (source.hasOwnProperty(propName)) {
          target[propName] = source[propName];
        }
      });
      target.__wrappedMethods = source.__wrappedMethods;
      zrUtil.each(CLONE_PROPERTIES, (propName) => {
