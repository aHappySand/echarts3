...
import { createHashMap, bind, each, hasOwn, map, clone, isObject, extend, isNumber } from '../../../zrender/core/util';
 ...
 */
const ExternalSource = /** @class */ (function () {
...
function applySingleDataTransform(transOption, upSourceList, infoForPrint,
  ...
  // Prepare source
  var extUpSourceList = map(upSourceList, (upSource) => createExternalSource(upSource, externalTransform));
  ...
  return map(resultList, (result, resultIndex) => {
    ...
    if (firstUpSource &&
      ...
      if (startIndex) {
        result.data = firstUpSource.data.slice(0, startIndex)
          .concat(result.data);
      }
      resultMetaRawOption = {
