...
function checkMissingComponents(option) {
  return;
  each(option, (componentOption, mainType) => {
...
const GlobalModel = /** @class */ (function (_super) {
  ...
  GlobalModel.prototype._mergeOption = function (newOption, opt) {
    ...
    each(newOption, (componentOption, mainType) => {
    ...
    if (replaceMergeMainTypeMap) {
      ...
      replaceMergeMainTypeMap.each((val, mainTypeInReplaceMerge) => {
  ...
  GlobalModel.prototype.getOption = function () {
    ...
    each(option, (optInMainType, mainType) => {
      if (ComponentModel.hasClass(mainType)) {
  ...
  GlobalModel.prototype.queryComponents = function (condition) {
    ...
    if (index != null) {
      result = [];
      each(modelUtil.normalizeToArray(index), (idx) => {
  ...
  GlobalModel.prototype.eachRawSeries = function (cb, context) {
    each(this._componentsMap.get('series'), (series) => {
  ...
  GlobalModel.prototype.restoreData = function (payload) {
    ...
    var componentsMap = this._componentsMap;
    var componentTypes = [];
    componentsMap.each((components, componentType) => {
      if (ComponentModel.hasClass(componentType)) {
        componentTypes.push(componentType);
      }
    });
    ComponentModel.topologicalTravel(componentTypes, ComponentModel.getAllClassMainTypes(), (componentType) => {
      each(componentsMap.get(componentType), (component) => {
        if (component &&
  ...
  GlobalModel.internalField = (function () {
    reCreateSeriesIndices = function (ecModel) {
      ...
      each(ecModel._componentsMap.get('series'), (series) => {
...
function queryByIdOrName(attr, idOrName, cmpts) {
  ...
  if (isArray(idOrName)) {
    ...
    each(idOrName, (idOrNameItem) => {
      if (idOrNameItem != null) {
        var idName = modelUtil.convertOptionIdName(idOrNameItem, null);
        idName != null && keyMap_1.set(idOrNameItem, true);
      }
    });
    return filter(cmpts, (cmpt) => cmpt && keyMap_1.get(cmpt[attr]));
  }
  var idName_1 = modelUtil.convertOptionIdName(idOrName, null);
  return filter(cmpts, (cmpt) => cmpt && idName_1 != null && cmpt[attr] === idName_1);
}
