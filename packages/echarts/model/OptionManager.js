 ...
 */
const OptionManager = /** @class */ (function () {
  ...
  OptionManager.prototype.setOption = function (rawOption, optionPreprocessorFuncs, opt) {
    if (rawOption) {
      ...
      each(normalizeToArray(rawOption.series), (series) => {
        series && series.data && isTypedArray(series.data) && setAsPrimitive(series.data);
      });
      each(normalizeToArray(rawOption.dataset), (dataset) => {
  ...
  OptionManager.prototype.getMediaOption = function (ecModel) {
    ...
    if (indices.length && !indicesEquals(indices, this._currentMediaIndices)) {
      result = map(indices, (index) => clone(index === -1 ? mediaDefault.option : mediaList[index].option));
    }
...
// `rawOption` May be modified
  ...
  function doPreprocess(option) {
    each(optionPreprocessorFuncs, (preProcess) => {
