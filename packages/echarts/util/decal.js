...
* under the License.
*/
import WeakMap from '../../zrender/core/WeakMap';
import LRU from '../../zrender/core/LRU';
import { defaults, map, isArray, isString, isNumber } from '../../zrender/core/util';
import { getLeastCommonMultiple } from './number';
import { createSymbol } from './symbol';
import { platformApi } from '../../zrender/core/platform';
...
function normalizeDashArrayX(dash) {
  ...
  for (let i = 0; i < dash.length; ++i) {
    if (isNumber(dash[i])) {
      ...
      var dashValue = map(dash[i], (n) => Math.ceil(n));
...
function normalizeDashArrayY(dash) {
  ...
  var dashValue = map(dash, (n) => Math.ceil(n));
...
function getLineBlockLengthX(dash) {
  return map(dash, (line) => getLineBlockLengthY(line));
}
