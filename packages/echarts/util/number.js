...
export function getPercentSeats(valueList, precision) {
  var sum = zrUtil.reduce(valueList, (acc, val) => acc + (isNaN(val) ? 0 : val), 0);
  if (sum === 0) {
    return [];
  }
  var digits = Math.pow(10, precision);
  var votesPerQuota = zrUtil.map(valueList, (val) => (isNaN(val) ? 0 : val) / sum * digits * 100);
  var targetSeats = digits * 100;
  var seats = zrUtil.map(votesPerQuota, (votes) =>
    // Assign automatic seats.
    Math.floor(votes));
  var currentSum = zrUtil.reduce(seats, (acc, val) => acc + val, 0);
  var remainder = zrUtil.map(votesPerQuota, (votes, idx) => votes - seats[idx]);
  ...
  return zrUtil.map(seats, (seat) => seat / digits);
}
