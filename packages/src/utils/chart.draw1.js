...
SceneManager.prototype = {
  ...
  drawXAxis(axisData, limit = { xLeft: 0, xRight: 0, zoom: 1 }) {
    ...
    axisData.forEach((axis) => {
      ...
      if (hasStartX) {
        const axisScale = this.countXAxisPos(limit, axis);
        xStartScale = axisScale.xStartScale;
        xEndScale = axisScale.xEndScale;
      }

      console.log(xStartScale, xEndScale, 'scale');
      ...
      if (type === 'category') {
        if (hasStartX) {
          startIndex = Math.floor(xStartScale * (len - 1));
          if (startIndex < 0) startIndex = 0;

          endIndex = Math.floor(xEndScale * (len - 1));
          if (endIndex >= len) {
            endIndex = len - 1;
          }
        }
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (i >= startIndex && i <= endIndex) {
          data[i].x = start;
          if ((i - startIndex) % step === 0 && start >= marginLeft) {
  ...
  drawPoint({ series, startSeriesIndex = 0, yAxisData, startSeriesDataIndex = 0, xAxisData, xPoses = null }) {
    if (series.length) {
      ...
      const mesh = this.createPointMesh({ code: 'why', count: total, size: 4 });
